...
class Alarm extends Component {
  ...
  // }
  checkAlert() {
    const { fireAlertsAlarm, location } = this.props;
    const { pathname } = location;
    const isDashboardPath = pathname === "/remote-monitoring/fire-dashboard";
    const isMapPath = pathname === "/remote-monitoring/map-dashboard";

    if (
      Number(fireAlertsAlarm?.length ?? 0) > 0 &&
      (isDashboardPath || isMapPath)
    ) {
      if (this.siren.paused) this.siren.play();
      this.setState((state) => ({ ...state, open: true }));
    } else {
